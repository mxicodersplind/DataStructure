...
class Common extends CI_Model {
    ...
    function getDataTableSource($tablename, $datatable_fields = array(), $conditions_array = array(), $data = '*', $request, $join_str = array(), $group_by = array()) {
        ...
        function getDataTableSourceCards($tablename, $datatable_fields = array(), $conditions_array = array(), $data = '*', $request, $join_str = array(), $group_by = array()) {
        ...
        foreach ($resultdata as $key => $resultdata_) {
            $query = $this->db->query("select * from dropurcard_card_share_log where card_id = ".$resultdata_['card_id']);
            $resultdata[$key]["total_shared"] = $query->num_rows();
            $query = $this->db->query("select * from dropurcard_card_view_log where card_id = ".$resultdata_['card_id']);
            $resultdata[$key]["total_view"] = $query->num_rows();
            $query = $this->db->query("select * from dropurcard_card_qrshare_log where card_id = ".$resultdata_['card_id']);
            $resultdata[$key]["total_qrshared"] = $query->num_rows();
        }
    ...
    function getDataTableSourceUser($tablename, $datatable_fields = array(), $conditions_array = array(), $data = '*', $request, $join_str = array(), $group_by = array()) {
        ...
        $resultdata =  $query->result_array();
        foreach ($resultdata as $key => $resultdata_) {
            $query = $this->db->query("SELECT * FROM dropurcard_card WHERE user_id=".$resultdata_['id']." AND card_type='mycards' AND card_create='regular'");
            $resultdata[$key]["cards"] = $query->num_rows();
            $query = $this->db->query("SELECT * FROM dropurcard_shared_card WHERE save_user_id=".$resultdata_['id']);
            $resultdata[$key]["saved"] = $query->num_rows();
            $query = $this->db->query("select * from dropurcard_events where user_id = ".$resultdata_['id']);
            $resultdata[$key]["organized_events"] = $query->num_rows();
            $query = $this->db->query("select * from dropurcard_event_dropcard where card_owner = ".$resultdata_['id']);
            $resultdata[$key]["attended_events"] = $query->num_rows();
            $query = $this->db->query("select * from dropurcard_card_share_log where card_shareperson_id = ".$resultdata_['id']);
            $resultdata[$key]["share"] = $query->num_rows();
            $query = $this->db->query("select * from dropurcard_card_view_log where card_viewer_id = ".$resultdata_['id']);
            $resultdata[$key]["view"] = $query->num_rows();
        }
